...
## 4. 批次 2：三级 Gate + 审核队列 + 定时发布队列 + 批准哈希 + 每日配额 (已完成, 2026-08-11)

**本批次是产品核心**，对标 Content OS 的"集中审核 → 发布队列"形态。

### 4.0 执行决策记录（2026-08-11）

- `chart_image_failed` 的判定时点存在现有架构边界：走势图生成目前发生在实际发布阶段，而 Gate 发生在内容生成阶段。为避免伪造成功或失败，本批次只在上游明确传入截图失败信号时写入该 reason；默认未知不等于失败，也不据此放行或阻断。后续如要在 Gate 前强制预生成截图，需要另行讨论缓存与重复网络请求策略。
- `PUBLISH_MIN_INTERVAL_MINUTES` 和 `PUBLISH_DAILY_QUOTA_PER_ACCOUNT` 作为全局默认值，但调度和计数始终按账号分别计算；本批次不新增未经方案定义的账号级覆盖键。
- 新调度时间使用运行主机本地时区 ISO；历史 `published_at` 若带其他时区偏移，配额统计先转换到本地自然日再计数，避免把 UTC 日期直接当成本地日期。
